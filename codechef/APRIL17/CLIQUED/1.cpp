...
void traverse(int node, ll dist_so_far, int parent) {
    ...
    if (paths.count(node)) {
        ...
        int i;
        for (i = 0; i < paths[node].size() && (not_old || paths[node][i].first < x); i++) {
        ...
        if (!not_old) {
            for (int j = 0; j < k; j++) {
                if (j != node) {
                    if (min_dist[j] > dist_so_far + x) {
                        min_dist[j] = (ll)(dist_so_far + x);
                        traverse(j, min_dist[j], node);
                    }
                }
            }
            while (i < paths[node].size()) {
                if (paths[node][i].second != parent) {
                    if (dist_so_far + (ll)paths[node][i].first < min_dist[paths[node][i].second]) 
                        traverse(paths[node][i].second, dist_so_far + (ll)paths[node][i].first, node);
                }
