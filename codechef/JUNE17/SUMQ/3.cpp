...
ll count_f() {
	...
	for (map<int, vector<bool> >::iterator it = v.begin(); it != v.end(); it++) {
		...
		if (it -> second[2]) { c2++; s2 = (s2 + (ll)it -> first) % M; }

		if (it -> second[0] && c2) {
			dp1 = (dp1 + c2) % M;
			dp2 = (dp2 + ((s2 + ((c2 * it -> first) % M)) % M)) % M;
			dp3 = (dp3 + ((s2 * it -> first) % M)) % M;
		}
		if (it -> second[2] && c0) {
			dp1 = (dp1 + c0) % M;
			dp2 = (dp2 + ((s0 + ((c0 * it -> first) % M)) % M)) % M;
			dp3 = (dp3 + ((s0 * it -> first) % M)) % M;
		}

		if (it -> second[0]) { c0++; s0 = (s0 + (ll)it -> first) % M; }
