...
map<pair<int, int>, vector<int> > path_nodes;   // <from_index, to_index>, <index, index, index...>

map<long long, bool> occurence;
...
int main() {
    ...
    get_subtree_path_nodes(node_book[1], NULL);
    //cout << "Made data";
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (in1 - 1) {
            *node_book[in2] = in3;
            ...
            vector<int> path;
            occurence.clear();
            path = path_nodes[{in2, in3}];
            //cout << endl << "path is: ";
            for (int i = 0; i < path.size(); i++) {
                //cout << path[i] << " ";
                occurence[node_book[path[i]] -> value] = true;
            }
            //cout << endl;
